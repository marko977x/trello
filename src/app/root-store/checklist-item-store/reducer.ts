...
import { ChecklistItemActionTypes, LoadChecklistItemsSuccess, ToggleChecklistItem, DeleteChecklistItem, SaveChecklistItemText, AddChecklistItem, ToggleChecklistItemSuccess, SaveChecklistItemTextSuccess, AddChecklistItemSuccess } from './actions';
...
function reducer(state = initialState, action: Action): ChecklistItemState {
  switch(action.type) {
    ...
    case ChecklistItemActionTypes.TOGGLE_CHECKLIST_ITEM_SUCESS: {
    ...
    case ChecklistItemActionTypes.SAVE_CHECKLIST_ITEM_TEXT_SUCCESS: {
      const {itemId, text} = (action as SaveChecklistItemTextSuccess);
      const update: Update<ChecklistItem> = {
        id: itemId,
        changes: {text}
      }
    ...
    case ChecklistItemActionTypes.ADD_CHECKLIST_ITEM_SUCCESS: {
      return ChecklistItemAdapter.addOne((action as AddChecklistItemSuccess).item, state);
    }
